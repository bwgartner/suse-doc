...
:OSDRAM: 128GB
:OSDCPUcount: 1
:OSDCPUmodel: Ampere Emag 8180 32Core 3.3GHz
:OSDOSDrivedesc: Micron 240GB M.2
:OSDDrivedesc: Micron 7300 NVMe, M.2 3.84TB
:OSDNic: Mellanox ConnectX-4 Dual Port 100GbE
:node2roles: Admin, monitor, and protocol gateways
:node2model: Lenovo HR330A
:node2CPUcount: 1
:node2RAM: 32GB
:node2OSDrivedesc: Micron 480GB
:node2YESlink: FixMe
:switchSpeed: 100Gb
:switchmodel: MSN3700C
:switchvendor: Mellanox
...
*{vendor} {vplatform}* (Revise)

{vendor} {vplatform} provide a cost effective and scaleable platform for the deployment of {SESProduct}.  
...
TIP:: Please note that limited use subscriptions are provided with {SESProduct} as part of the subscription entitlement
...
* Repository Mirroing Tool (RMT) for SLES - allows enterprise customers to optimize the management of SUSE Linux Enterprise (and extensions such as {SESProduct}) software updates and subscription entitlements. It establishes a proxy system for SUSE Customer Center with repository and registration targets.
...
|Monitor |amp-mod3.suse.lab |172.16.227.63 |N/A
...
== Conclusion
The {vendor} {vplatform} series represents a strong capacity-oriented platform.  When combined with the access flexibility and reliability of {SESProduct} and the industry leading support from {vendor}, any business can feel confident in the ability to address the exponential growth in storage they are currently faced with.
...
== Appendix E: Performance Data (FixMe - massive updates)
