...
== Executive Summary
This reference configuration is intended to help an organization plan and install an on-premise container-as-a-service platform infrastructure to deploy, orchestrate and manage containerized workloads.
...
=== Network
Networking and the associated services are the technology components which typically requires the most advanced planning. Connectivity, capacity and bandwidth requirements for a container-as-a-service infrastructure have a fair amount of complexity, especially when integrated with an existing IT infrastructure and accounting for the magnitude of microservices being deployed.
...
HAProxy Setup::
Either a load-balancer or HAProxy is a suggested approach to make the cluster, and more specifically some of the core Kubernetes Master Node API functions, accessible to client systems. It does this via a virtual IP address which then sends the call to any active masters. While not required for a single master cluster, setting this up in advance allows later expansion and substitutions to happen.
