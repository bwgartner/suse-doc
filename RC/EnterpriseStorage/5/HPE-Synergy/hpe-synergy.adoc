...
* Include only the minimal pattern and components, according to the procedure from deployment. This can be accomplished in any number of ways: with the virtual media option through iLO, or from a PXE network-boot environment.
...
then execute 'systemctl reload multipathd', re-run the previous 'proposal.populate' or manually modify the OSD node minion files to reflect the drive designation of one I/O path or the other.
...
||4+|??|{IHVPartner} {IHVPlatform} xGB SSD SAS drives| match the number of OSD nodes and in the ratio of 1 SSD : 5-7 OSD drives
||28+|??|{IHVPartner} {IHVPlatform} xGB SAS drives| suggested minimum of 8 drives per OSD node
|===
