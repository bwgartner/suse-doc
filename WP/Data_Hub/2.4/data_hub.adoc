...
.Revisions
[options="header",cols="^s,<d,<s"]
|=======================
|Version |Date         |Comments
|0.5     |01/15/19     |Initial draft
|0.6     |02/11/19     |Collaborative draft
|1.0     |03/05/19     |First draft to be shared with HPE as SUSE Company Confidential
|1.1     |03/31/19     |Proposed hardware config - shared w/ HPE as SUSE Company Confidential
|=======================
...
Before implementing a data analytics solution, you need to consider the specific problem you are working to solve. Below are some use cases that help you to zero in on the type of solution you are pursuing.

== What The Fuck

=== Example Use Cases
This section will outline a few potential use cases for SAP Data Hub build on SUSE Containers as a Service Application Platform.   In general, SAP Data Hub exceeds in pulling information from multiple kinds of internal and external data resources to enable insight into very complex analytical problems.   The use of machine learing and Big Data analytics platforms like SAP, Hadoop, MapR, Cloudera, etc. require access to large pools of unstructured data in a systematic, highly automated, secure way.
...
|Deployment            |Easy deployment, using a proven-to-work combination of the several components.
...
With a solid understanding of your requirements, you can now begin to design the solution.  The following section will outline the key concepts in the software architecture of the {ISVPartner} {ISVSolution} reference configuration.
...
ifeval::["{IHVSummary}" == "IHVsummary"]

include::{IHVSummary}.adoc[]
...
== Bill of Materials
The HPE ProLiant DL360 series Gen10 server is an excellent platform for a proof-of-concept implementation of SAP Data Hub on SUSE CaaS Platform software.  The DL360 provides powerful, dense compute and storage capabilities via the servers selected for this reference architecture.
And the ProLiant iLO management capability is a key piece of the management infrastructure for a data analytics cluster.

=== HPE ProLiant DL360 Gen10 Servers
// This table came from the HOS8 RA and was updated for CaaSP.
// What about the Gen10 server line?
