...
+
* To customize virtual hardware, adjust _CPU_, _Memory_, _New Hard Size_ values to match at least the minimum requirement quantity/sizes of the target solution role
+
NOTE: Refer to respective https://documentation.suse.com/[{CompanyName} product documentation] for details on the node requirements
+
...
+
** Expand _CD/DVD Drive 1_ and adjust settings
...
*** Check _Connect At Power On_ then *OK*
+
[[img-prep-VM_CD]]
